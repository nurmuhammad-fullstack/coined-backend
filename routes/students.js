...
// ── POST /api/students/:id/coins ── add or remove coins
router.post('/:id/coins', protect, teacherOnly, async (req, res) => {
  try {
    ...
    if (type === 'spend') {
      if (student.coins < amount) return res.status(400).json({ message: 'Not enough coins' });
...
router.get('/:id/transactions', protect, async (req, res) => {
  try {
    ...
    if (!isOwn && !isTeacher) return res.status(403).json({ message: 'Access denied' });
